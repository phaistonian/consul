...
| [Blocking Queries](/api-docs/features/blocking) | not supported from commands, but may be from the corresponding HTTP API endpoint |
| [Agent Caching](/api-docs/features/caching) | not supported from commands, but may be from the corresponding HTTP API endpoint |
...
| Characteristic | Value |
| :------------- | :---- |
| [Required ACLs](/docs/security/acl/acl-system) | `operator:read`; however, a non-anonymous token can always read its own partition |

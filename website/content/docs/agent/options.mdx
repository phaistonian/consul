...
- TLS Configuration
  ...
  - To avoid a possible security issue, Some parameters part of the TLS Configuration are not automatically reloaded when [-auto-reload-config](#_auto_reload_config) is enabled.
    - [-encrypt-verify-incoming](#_encrypt_verify_incoming)
    - [-verify-incoming](#_verify_incoming)
    - [-verify-incoming-rpc](#_verify_incoming_rpc)
    - [-verify-incoming-https](#_verify_incoming_https)
    - [-verify-outgoing](#_verify_outgoing)
    - [-verify-server-hostname](#_verify_server_hostname)
    - [-ca-file](#_ca_file)
    - [-ca-path](#_ca_path)
    ...
    A manual call to `consul reload` or a issuing a `SIGHUP` to Consul process would reload the new values.
- Watches
